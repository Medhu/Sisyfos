...
package Server.Server is
   ...
   type Type_Game_End_Procedure is access procedure (P_Game_Status : out Status.Type_Game_Status);
...
-- what map they are playing on and how much resources they have at the moment.
      entry Entry_Get_Server_Info (P_Server_Info : out Utilities.RemoteString_List.Vector;
         P_Status                                : out Status.Type_Adm_Status);

      -- Client code can change Server_Info
      entry Entry_Set_Server_Info (P_Server_Info : in     Utilities.RemoteString_List.Vector;
         P_Status                                :    out Status.Type_Adm_Status);
      ...
      entry Entry_Create_Game (P_Create_File_Name : in     Utilities.RemoteString.Type_String;
      ...
      entry Entry_Save_Game (P_Save_File_Name : in     Utilities.RemoteString.Type_String;
         P_Status                             :    out Status.Type_Adm_Status);
      ...
      entry Entry_Load_Game (P_Load_File_Name : in     Utilities.RemoteString.Type_String;
         P_Status                             :    out Status.Type_Adm_Status);
      ...
      entry Entry_Join_Game (P_Player_Id :    out Player.Type_Player_Id;
         P_Player_Name                   : in     Utilities.RemoteString.Type_String;
         P_Status                        :    out Status.Type_Adm_Status);

      entry Entry_Leave_Game (P_Player_Id : in     Player.Type_Player_Id;
         P_Player_Name                    : in     Utilities.RemoteString.Type_String;
         P_Status                         :    out Status.Type_Adm_Status);

      entry Entry_Get_Player_Name (P_Player_Id : in     Player.Type_Player_Id;
         P_Player_Name                         :    out Utilities.RemoteString.Type_String;
         P_Status                              :    out Status.Type_Adm_Status);

      entry Entry_Get_Activity_Reports (P_Player_Id : in     Player.Type_Player_Id;
         P_Activity_Report_List :    out Observation.Activity.Activity_Report.Vector);

      entry Entry_Get_Pieces_Report (P_Player_Id : in     Player.Type_Player_Id;
         P_Visibility_Frames :    out Observation.Frames.Piece_Visibility_Frames.Vector);

      entry Entry_Create_Piece (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Pos : in Hexagon.Type_Hexagon_Position;
         P_Piece                            : in Piece.Type_Piece);

      entry Entry_Put_Piece (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Pos : in Hexagon.Type_Hexagon_Position;
         P_Piece_Id                      : in Piece.Type_Piece_Id);

      entry Entry_Remove_Piece (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id);

      entry Entry_Perform_Attack (P_Player_Id      : in Player.Type_Player_Id;
         P_Action_Type                             : in Action.Type_Action_Type;
         P_Attacking_Piece_Id, P_Attacked_Piece_Id : in Piece.Type_Piece_Id);

      entry Entry_Perform_Ranged_Attack (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type                               : in Action.Type_Action_Type;
         P_Attacking_Piece_Id, P_Attacked_Piece_Id   : in Piece.Type_Piece_Id);

      entry Entry_Perform_Move (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_To_Pos                           : in Hexagon.Type_Hexagon_Position);

      entry Entry_Perform_Patch_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect_Name : in Effect.Type_Effect_Name; P_Area : in Hexagon.Area.Type_Action_Capabilities_A);

      entry Entry_Perform_Piece_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect_Name                                   : in Effect.Type_Effect_Name);

      entry Entry_Grant_Piece_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect                                 : in Effect.Type_Effect);

      entry Entry_Revoke_Piece_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect_Name                             : in Effect.Type_Effect_Name);

      entry Entry_Grant_Patch_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect : in Effect.Type_Effect; P_Area : in Hexagon.Area.Type_Action_Capabilities_A);

      entry Entry_Revoke_Patch_Effect (P_Player_Id : in Player.Type_Player_Id;
         P_Action_Type : in Action.Type_Action_Type; P_Piece_Id : in Piece.Type_Piece_Id;
         P_Effect_Name                             : in Effect.Type_Effect_Name;
         P_Area                                    : in Hexagon.Area.Type_Action_Capabilities_A);

      -- The client will call this frequently to get some system data (mostly)
      entry Entry_Get_Updates_Summary (P_Player_Id : in     Player.Type_Player_Id;
      ...
      entry Entry_Get_Game_Engine_State (P_Game_Engine_State : out Status.Type_Engine_State);
   ...
   procedure Game_Engine_Free is new Unchecked_Deallocation (Type_Game_Engine,
