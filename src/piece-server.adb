...
package body Piece.Server is
   ...
   function Find_Piece_In_List
     (P_Piece_Id : in Piece.Type_Piece_Id) return Piece.Server.Pieces_Server_List.Cursor
   is
   ...
   begin
      ...
      while Piece.Server.Pieces_Server_List.Has_Element (Trav) and not Found loop
         if Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.all.Id = P_Piece_Id then
            Found := True;
         else
      ...
      if not Found then
         raise Piece_Not_Found_Piece_Position with "Did not find Piece.Id:" & P_Piece_Id'Img;
      end if;
   ...
   function Left_Less_Pieces (Left, Right : in Type_Piece_Access_Class) return Boolean is
   begin
      return Integer (Left.Id) < Integer (Right.Id);
   end Left_Less_Pieces;

   function Equal_Pieces (Left, Right : in Type_Piece_Access_Class) return Boolean is
   begin
      return Left = Right;
   end Equal_Pieces;
   ...
   begin
      ...
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Generate_Piece_Id - enter - exit Next_Id := " & Next_Id'Img);
      end if;
   ...
   begin
      ...
      if P_Piece.Category = Piece.House_Piece then
         P_Server_Piece :=
           new Piece.Server.Type_Piece'Class'(Piece.Server.House_Piece.House_Class.all);
   ...
   procedure Init_Piece (P_Server_Piece : in out Piece.Server.Type_Piece_Access_Class) is
   begin
      if Verbose then
   ...
   procedure Delete_Piece (P_Server_Piece : in out Piece.Server.Type_Piece_Access_Class) is
   ...
   function Count_Positions return Integer is   -- service to unit tests to be able to teste
      --Reset_Positions;
   ...
   begin
      ...
      while Piece.Server.Pieces_Server_List.Has_Element (Trav) loop
         ...
         if Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.P_Valid then
      ...
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Count_Positions - exit " & Num_Of_Positions'Img);
      end if;

      return Num_Of_Positions;
   end Count_Positions;

   procedure Set_Position (P_Piece : in Type_Piece; P_Pos : in Hexagon.Type_Hexagon_Position) is
      Trav                  : Piece.Server.Pieces_Server_List.Cursor;
   ...
   begin
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Set_Position - enter Piece.Id=" & P_Piece.Id'Img);
         if P_Pos.P_Valid then
            Text_IO.Put_Line (" P_Pos.A=" & P_Pos.A'Img & " P_B=" & P_Pos.B'Img);
         else
      ...
      Piece.Server.Pieces_Server_List.Replace_Element
        (All_Pieces_In_Game, Trav, Actual_Piece_Position);
   ...
   procedure Unset_Position (P_Piece : in Type_Piece; P_Pos : in Hexagon.Type_Hexagon_Position) is
      Trav                  : Piece.Server.Pieces_Server_List.Cursor;
      Actual_Piece_Position : Type_Piece_Position;
   ...
   begin
      if Verbose then
         Text_IO.Put ("Piece.Server.Unset_Position - enter Piece.Id=" & P_Piece.Id'Img);
         if P_Pos.P_Valid then
            Text_IO.Put_Line (" P_Pos.A=" & P_Pos.A'Img & " P_B=" & P_Pos.B'Img);
         else
            Text_IO.Put_Line ("P_Pos=Invalid");
      ...
      Actual_Piece_Position.Actual_Pos := Hexagon.Type_Hexagon_Position'(P_Valid => False);
      Piece.Server.Pieces_Server_List.Replace_Element
        (All_Pieces_In_Game, Trav, Actual_Piece_Position);
   ...
   procedure Put_All_Pieces_Positions (P_Player_Id : in Player.Type_Player_Id) is
      Trav : Piece.Server.Pieces_Server_List.Cursor;
           ...
           Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.P_Valid then
            Text_IO.Put
              ("All_Pieces_In_Game(Trav).Actual_Piece.Id=" &
               ...
               Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.A'Img &
               "All_Pieces_In_Game(Trav).Actual_Pos.B=" &
               Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.B'Img);
         end if;
         Trav := Piece.Server.Pieces_Server_List.Next (Trav);
      end loop;
   end Put_All_Pieces_Positions;

   function Get_All_Pieces_Positions return Piece.Server.Pieces_Server_List.Vector is
   begin
   ...
   begin
      ...
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Get_Effects_On_Piece - enter P_Piece.Id=" & P_Piece.Id'Img);
      end if;
      ...
      while Effect.Effect_List.Has_Element (Effect_Cursor) loop
         ...
         Observation.Observation_Of_Pieces_Effects.Observations_Of_Pieces_Effects.Include
           (P_Pieces_Effect,
   ...
   begin
      ...
      while Effect.Effect_List.Has_Element (Trav_Patch_Effects) loop
         ...
         Observation.Observation_Of_Patches_Effects.Observations_Of_Patches_Effects.Include
           (P_Observed_Patches_Effects,
            Observation.Observation_Of_Patches_Effects.Type_Patch_Effect'
              (P_Patch.Pos, Effect.Effect_List.Element (Trav_Patch_Effects), True));
   ...
   is
      ...
      Pos_A, Pos_B                                 : Integer;
      Patch_Observer_Adress, Patch_Observed_Adress : Hexagon.Server_Map.Type_Server_Patch_Adress;
   ...
   begin
      ...
      Trav_All_Pieces := Piece.Server.Pieces_Server_List.First (All_Pieces_In_Game);
         ...
         if Piece.Server.Pieces_Server_List.Element (Trav_All_Pieces).Actual_Piece.Player_Id =
           P_Player_Id then
            ...
            if Piece.Server.Pieces_Server_List.Element (Trav_All_Pieces).Actual_Pos.P_Valid then
               ...
               Observing_Piece_Pos :=
                 Piece.Server.Pieces_Server_List.Element (Trav_All_Pieces).Actual_Pos;

               if Verbose then
                  Text_IO.Put_Line
                    ("Piece.Server.Get_Pieces_Report - calculate area for piece at A=" &
                     Observing_Piece_Pos.A'Img & " B=" & Observing_Piece_Pos.B'Img);
               end if;

               Patch_Observer_Adress :=
                 Hexagon.Server_Map.Get_Patch_Adress_From_AB
                   (Observing_Piece_Pos.A, Observing_Piece_Pos.B);

               Trav_Observing_Pieces :=
                 Landscape.Pieces_Here_List.First (Patch_Observer_Adress.all.Pieces_Here);
               while Landscape.Pieces_Here_List.Has_Element (Trav_Observing_Pieces) loop
                  Observing_Piece :=
                    Piece.Server.Find_Piece_In_List
                      (Landscape.Pieces_Here_List.Element (Trav_Observing_Pieces))
                      .Actual_Piece;
                  ...
                  for Trav_Obs in Area_To_Check'First .. Area_To_Check'Last loop
                     ...
                     then
                        ...
                        Trav_Observed_Pieces :=
                          Landscape.Pieces_Here_List.First (Patch_Observed_Adress.all.Pieces_Here);

                        while Landscape.Pieces_Here_List.Has_Element (Trav_Observed_Pieces) loop
                           Observed_Piece :=
                             Piece.Server.Find_Piece_In_List
                               (Landscape.Pieces_Here_List.Element (Trav_Observed_Pieces))
                               .Actual_Piece;
                           ...
                           Trav_Observed_Pieces :=
                             Landscape.Pieces_Here_List.Next (Trav_Observed_Pieces);
                  ...
                  Trav_Observing_Pieces := Landscape.Pieces_Here_List.Next (Trav_Observing_Pieces);
   ...
   function Get_Pieces_Players (P_Patch : in Landscape.Type_Patch) return Player.Type_Player_Id is
   ...
   begin
      ...
      Trav := Landscape.Pieces_Here_List.First (P_Patch.Pieces_Here);      ---only one player
      --can be on any
      --patch, so it is
      --enough to test
      --one of them
      A_Piece :=
        Piece.Server.Find_Piece_In_List (Landscape.Pieces_Here_List.Element (Trav)).Actual_Piece;
      Ret := A_Piece.all.Player_Id;

      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Get_Pieces_Players -  Piece_Id=" & A_Piece.all.Id'Img & " Player_Id=" &
            Ret'Img);
      end if;
   ...
   begin
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Fighting_Piece.Is_Players_Piece - enter P_Piece.Player_Id=" &
            P_Piece.Player_Id'Img & " P_Player_Id=" & P_Player_Id'Img);
      end if;
      ...
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Fighting_Piece.Is_Players_Piece - exit");
      end if;
   ...
   begin
      ...
      while Landscape.Pieces_Here_List.Has_Element (Trav) and not Found loop
         A_Piece :=
           Piece.Type_Piece
             (Piece.Server.Find_Piece_In_List (Landscape.Pieces_Here_List.Element (Trav))
                .Actual_Piece.all);
   ...
   begin
      ...
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Patch_Belongs_To_Player - exit " & Ret'Img);
      end if;
   ...
   begin
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Validate_Target_Patch_And_Piece - enter");
      end if;
      P_Status := Status.Ok;
      ...
      if not Landscape.Server.Has_Patch_Free_Slot (P_Patch) or
        ...
        (not Piece.Server.Fighting_Piece.Can_Move_Here
           (P_Piece.Type_Of_Piece, P_Patch.Landscape_Here))
      then
      ...
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Validate_Target_Patch_And_Piece - exit, status=" & P_Status'Img);
      end if;
   ...
   begin
      ...
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Validate_Grant_Revoke_Effect_Piece - exit");
      end if;
   ...
   begin
      if Verbose then
         if P_Patch.Pos.P_Valid then
            Text_IO.Put_Line
              ("Piece.Server.Put_Piece the piece in patch  P_A=" & P_Patch.Pos.A'Img & "  P_B=" &
               P_Patch.Pos.B'Img);
         else
            Text_IO.Put_Line ("Piece.Server.Put_Piece the piece in patch  invalid position");
         end if;
      end if;
   ...
   begin
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Remove_Piece the piece in patch  P_Piece.Id=" & P_Piece.Id'Img);
      end if;
   ...
   begin
      ...
      if not Piece.Server.Validate_Grant_Revoke_Effect_Piece (P_Piece, P_Player_Id) then
         P_Status := Status.Not_Players_Piece;
      else
         Effect.Effect_List.Include (P_Piece.Effects_On_Piece, P_Effect.Effect_Name, P_Effect);
         P_Status := Status.Ok;
      end if;
   ...
   begin
      ...
      if not Piece.Server.Validate_Grant_Revoke_Effect_Piece (P_Piece, P_Player_Id) then
         ...
         Effect.Effect_List.Exclude (P_Piece.Effects_On_Piece, P_Effect_Name);
         P_Status := Status.Ok;
      end if;
   ...
   begin
      ...
      if Verbose then
         Text_IO.Put_Line ("Piece.Server.Validate_Grant_Revoke_Effect_Patch - exit");
      end if;
   ...
   begin
      if Verbose then
         Text_IO.Put_Line
           ("Piece.Server.Grant_Patch_Effect- enter Effect " & P_Effect.Effect_Name'Img & " Aux=" &
            P_Effect.Aux'Img);
      end if;

      if not Piece.Server.Validate_Grant_Revoke_Effect_Piece (P_Piece, P_Player_Id) then
         P_Status := Status.Not_Players_Piece;
      elsif not Piece.Server.Validate_Grant_Revoke_Effect_Patch (P_Piece, P_Player_Id, P_Area) then
         ...
         for Trav_Area in P_Area'First .. P_Area'Last loop
            if P_Area (Trav_Area).P_Valid then
               A_Patch :=
                 Hexagon.Server_Map.Get_Patch_Adress_From_AB
                   (P_Area (Trav_Area).A, P_Area (Trav_Area).B);

               Effect.Effect_List.Include (A_Patch.Effects_Here, P_Effect.Effect_Name, P_Effect);
            end if;
         end loop;
         P_Status := Status.Ok;
      end if;
      ...
      if not Piece.Server.Validate_Grant_Revoke_Effect_Piece (P_Piece, P_Player_Id) then
         ...
         for Trav_Area in P_Area'First .. P_Area'Last loop
            if P_Area (Trav_Area).P_Valid then
               ...
               Effect.Effect_List.Exclude (A_Patch.Effects_Here, P_Effect_Name);
            end if;
         end loop;
         P_Status := Status.Ok;
      end if;
   ...
   begin
      Trav := Piece.Server.Pieces_Server_List.Last (Piece.Server.All_Pieces_In_Game);
      Text_IO.Put_Line
        ("Printing Pieces In Game Length =" &
         Piece.Server.Pieces_Server_List.Length (Piece.Server.All_Pieces_In_Game)'Img);
      while Piece.Server.Pieces_Server_List.Has_Element (Trav) loop

         Text_IO.Put
           ("Index=" & Piece.Server.Pieces_Server_List.To_Index (Trav)'Img & " Id=" &
            Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.Id'Img & " Type_Of_Piece=" &
            Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.Type_Of_Piece'Img &
            " Player_Id=" &
            Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.Player_Id'Img);
         if Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.P_Valid then
            Text_IO.Put (" A=" & Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.A'Img);
            Text_IO.Put (" B=" & Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.B'Img);
         else
         ...
         Effect.Print_Effect_List
           (Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.all.Effects_On_Piece);
   ...
   procedure Upkeep_All (P_Player_Id : in Player.Type_Player_Id) is
      A_Patch : Hexagon.Server_Map.Type_Server_Patch := Hexagon.Server_Map.Empty;
      A_Piece : Piece.Server.Type_Piece_Access_Class;
      Trav    : Piece.Server.Pieces_Server_List.Cursor;
            ...
            A_Patch :=
              Hexagon.Server_Map.Get_Patch_Adress_From_AB
                (Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.A,
                 Piece.Server.Pieces_Server_List.Element (Trav).Actual_Pos.B).all;
            A_Piece :=
            ...
            if A_Piece.all.Player_Id = P_Player_Id then
               ...
               if Piece.Server.Pieces_Server_List.Element (Trav).Actual_Piece.Category =
                 Piece.Fighting_Piece then
                  ...
                  exception
                     when others =>
                        Text_IO.Put_Line (Text_IO.Current_Error, "Piece.Server.Upkeep_All:");
                        Text_IO.Put_Line
   ...
   begin
      ...
      Ada.Streams.Stream_IO.Create
        (Write_File, Ada.Streams.Stream_IO.Out_File, Ada.Strings.Unbounded.To_String (P_File_Name));
   ...
   begin
      ...
      Text_IO.Put_Line ("Pieces File Name:" & Ada.Strings.Unbounded.To_String (P_File_Name));

      Ada.Streams.Stream_IO.Open
        (Read_File, Ada.Streams.Stream_IO.In_File, Ada.Strings.Unbounded.To_String (P_File_Name));
